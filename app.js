...
// parameter1是要控制的對象
// parameter2是duraction
// parameter3是控制對象的原始狀態
// parameter4是控制對象動畫結束後的狀態
// parameter5
time_line
...
function setGPA() {
  ...
  if (creditSum == 0) {
    ...
    result = (sum / creditSum).toFixed(2); //只到小數點第二位
  }
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    let className = graders[i].children[0].value; //class category
    let classNumber = graders[i].children[1].value; //class number
    let classCredit = graders[i].children[2].value; //credit
    let classGrade = graders[i].children[3].value; //grade
    // console.log(className, classNumber, classCredit, classGrade);
  ...
  //取得objectArray後，將成績grade從string轉為數字
  for (let i = 0; i < objectArray.length; i++) {
    objectArray[i].classGradeNumber = convertor(objectArray[i].classGrade);
  }
